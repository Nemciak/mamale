...
module.exports = {
  config: {
    name: "wings3d",
    description: "Nadawanie skrzydel3d",
  },
  run: async (client, message, args) => {
    ...
    let cape = args[0];
      ...
      minecraftusers[message.author.id] = {
        nickname: null
          ...
          const polish_gived = new MessageEmbed()
              .setColor('#00ff00')
              .setAuthor(`✅ Skrzydla 3d zostały nadane!`)
              .setDescription(`ID skrzydel 3d: **${cape}**\nNick na który została nadana: **${minecraftusers[message.author.id].nickname}**`)
          ...
          const polish_notr = new MessageEmbed()
              ...
              .setFooter(`${config["Konfiguracja"].name}    •   Użyte przez: ${message.author.tag}`, client.user.displayAvatarURL());
                      ...
                      /// EMBEDY ///
                      if (message.member.roles.cache.has("931983779843670116")) {
      ...
      return;
              ...
              if (fs.existsSync(`/var/www/html/cosmetic_list/wings_3did/${cape}.png`)) {
                  fs.copyFile(`/var/www/html/cosmetic_list/wings_3did/${cape}.png`, `/var/www/html/items/wings3d/users/${minecraftusers[message.author.id].nickname}.png`, (err) => {
                      if (err) throw err;
                  });
